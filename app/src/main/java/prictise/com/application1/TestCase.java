...
/**
 @Test public int reversal(int n) {
 int ret = 0;
 ...
 @Test public int[] addTwoNum(int[] nums, int target) {
 int len = nums.length;
 HashMap<Integer, Integer> temp = new HashMap<>(len - 1);
 temp.put(nums[0], 0);
 for (int i = 1; i < len; i++) {
 ...
 @Test public void producterCusumer() {
 ...
 // 这是写入异常
 @Test public void bufferOverflowException() {
 ...
 // 这是读取异常
 @Test public void bufferUnderflowException() {
 //  这里只分配了2个字节，下面的params.put(tmp);却put了3个字节的数据。
 // 所以导致 java.nio.BufferOverflowException 异常
 ByteBuffer params = ByteBuffer.allocate(2);
 params.order(ByteOrder.LITTLE_ENDIAN);
 ...
 @Test public void deleteArrayElements() {
 int[] s = {5, 7, 2, 9};
 ...
 @Test public void printFileName() {
 ...
 @Test public void testMapRemoveKey() {
 ...
 @Test public void json() throws JSONException {
 ...
 @Test public void dateStringToMillis() {
 ...
 @Test public void testLinkedAccessFalseHashMap() { //
 ...
 @Test public void testLinkedAccessTrueHashMap() { //
 // false 是访问排序 true 是插入排序
